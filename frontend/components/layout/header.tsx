  ...
  return (
    <header className="h-14 border-b border-border bg-card flex items-center justify-between px-4 sticky top-0 z-40">
      <div className="flex items-center gap-3">
        <button onClick={toggleSidebar} className="lg:hidden p-1.5 rounded-md hover:bg-accent">
        ...
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
          {breadcrumbs.map((b, i) => (
            <span key={i} className="flex items-center gap-1.5">
              ...
              <span className={cn("hover:text-foreground cursor-pointer", i === breadcrumbs.length - 1 && "text-foreground font-medium")}>
                {b.label}
              </span>
            </span>
          ))}
        ...
        <div className="relative">
          ...
          {userMenuOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-48 rounded-md border border-border bg-card shadow-lg z-50 py-1">
